...
<body>
  ...
  <main id="main" class=" bg-dark">
  		...
  		<div id="login-right">
  			<div class="card col-md-8 glass">
  				<div class="card-body">
  						
  					<form id="login-form" >
					  	<input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token']; ?>">
  						<div class="form-group">
  							<label for="username" class="control-label" style="color:#fff;">Username</label>
		...
		$('#login-form button[type="button"]').removeAttr('disabled').html('Login');
			...
			success:function(resp){
				...
				if(resp == 1){
					...
					$('#login-form').prepend('<div class="alert alert-danger">Invalid token</div>');
					$('#login-form button[type="button"]').removeAttr('disabled').html('Login');
				}else{
